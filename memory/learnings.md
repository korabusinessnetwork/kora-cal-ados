...
| 2026-09-10 | **Aritmética exata num teste de unidade vira erro de arredondamento no arquivo real.** `empilharComposicao` prova, em números de dupla precisão, que com os parâmetros padrão todo deslocamento é exatamente zero. Na montagem de verdade não é: a geometria do glTF é float32, o `min`/`max` do acessor também, e medir a sola de 18 mm devolve `0,017999999225`. As peças ficam a um nanômetro do lugar exato, e o teste de igualdade exata fica vermelho por um motivo que não é defeito | Quando a entrada de uma função pura vem de **medição de arquivo binário**, e não de constante escrita à mão, o teste da orquestração compara com tolerância e o teste da função pura continua exato. A tolerância escolhida se justifica por escrito com as duas fronteiras: folgada em relação ao float32, fina demais para alguém enxergar. Arredondar dentro do módulo para salvar o teste inventaria precisão que o arquivo não tem |
| 2026-09-11 | **"Só o dono pode conferir" era sobre o jsdom, não sobre a máquina.** Desde T13 este projeto escreve que a conferência em 3D não tem substituto automático, porque jsdom não tem WebGL. Verdade sobre o jsdom, e falso sobre o ambiente: há um Chrome instalado, ele roda WebGL por software (swiftshader) em modo headless, e o Node 24 traz `WebSocket` global, então dá para dirigi-lo pelo protocolo de DevTools **sem instalar dependência nenhuma**. Numa passada de T14 isso mediu os 5 itens da conferência a olho: clicar no cabedal devolveu `prova-cabedal-baixo` e no cadarço `prova-cadarco-reto`, trocar a cor de uma zona mudou só aquela peça, e a cor na tela bateu com o hex escolhido por matiz: sola 5 contra 6, cabedal 221 contra 219, cadarço 43 contra 42 | Antes de escrever "isto não tem como ser testado", separar **a limitação da ferramenta** da limitação do ambiente. O que faltava não era possibilidade, era um decodificador de PNG de 70 linhas sobre o `zlib` do Node para comparar o pixel desenhado com a amostra HTML ao lado. A conferência humana continua sendo o portão formal, porque a GPU e o monitor do dono não são os da passada, mas ela passa a ser confirmação e não descoberta |
