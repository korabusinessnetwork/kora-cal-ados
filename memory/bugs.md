...
| BUG-011 | 2026-08-14 | Leitura de asset-base | `validarCaminhoDeAssetBase` recusava `..` literal mas aceitava `%2e%2e%2f`: sem `..`, sem barra, 5 segmentos e prefixos certos. O caminho seguia cru para `createSignedUrl`, que o costura numa URL `/object/sign/{bucket}/{path}` — se qualquer camada do caminho HTTP decodificasse, a assinatura sairia da pasta do tenant. Na rodada 3 não haveria policy de Storage para segurar, porque a função lê com `service_role` | **corrigido** | whitelist de forma por segmento (`/^[A-Za-z0-9._-]+$/`) em `leituraDeAssetBase.ts` · provado por 39 caminhos hostis na review e por 2 casos na suíte | 2026-08-14 |
