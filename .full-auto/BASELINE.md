...
| Medida | Abertura do refino | Fechamento da rodada 1 | Fechamento da rodada 2 | Fechamento da rodada 3 | Fechamento da rodada 4 | Fechamento da rodada 5 | Fechamento da rodada 6 | Fechamento da rodada 7 | Fechamento da rodada 8 | Fechamento da rodada 9 | Fechamento da rodada 10 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| Testes verdes | **1047** passando, 58 pulados, 70 arquivos | **1072** passando, 58 pulados, 71 arquivos | **1104** passando, 58 pulados, 74 arquivos | **1161** passando, 58 pulados, 80 arquivos | **1198** passando, 58 pulados, 85 arquivos | **1243** passando, 58 pulados, 92 arquivos | **1275** passando, 58 pulados, 97 arquivos | **1308** passando, 58 pulados, 101 arquivos | **1329** passando, 58 pulados, 105 arquivos | **1343** passando, 58 pulados, 107 arquivos | **1346** passando, 58 pulados, 108 arquivos |
| Testes contra o banco real | 58 de 58 | **58 de 58** | **58 de 58** | **58 de 58** | **58 de 58** | **58 de 58** | **58 de 58** | **58 de 58** | **58 de 58** | **58 de 58** | **58 de 58** |
| Testes em navegador | 25 | **25** | **25** | **25** | **25** | **25** | **25** | **25** | **25** | **25** | **25** |
| `tsc --noEmit` | limpo | limpo | limpo | limpo | limpo | limpo, e agora olhando **dois diretórios a mais** (A45) | limpo | limpo | limpo | limpo | limpo |
| `npm run build` | sem erro, **com aviso de chunk**, 695 ms | sem erro, **com aviso de chunk**, **419 ms** | sem erro, **com aviso de chunk**, **507 ms** | sem erro, **com aviso de chunk**, **564 ms** | sem erro, **com aviso de chunk**, **512 ms** | sem erro, **com aviso de chunk**, **483 ms** | sem erro, **com aviso de chunk**, **506 ms** | sem erro, **com aviso de chunk**, **439 ms** | **limpo, sem erro e sem aviso** (A61), **409 ms** | limpo, sem erro e sem aviso, **388 ms** | limpo, sem erro e sem aviso, **566 ms** |
| `npm audit` | 0 vulnerabilidades | **0 vulnerabilidades** | **0 vulnerabilidades** | **0 vulnerabilidades** | **0 vulnerabilidades** | **0 vulnerabilidades** | **0 vulnerabilidades** | **0 vulnerabilidades** | **0 vulnerabilidades** | **0 vulnerabilidades** | **0 vulnerabilidades** |
| Bundle: chunk principal | 455,03 kB (gzip 131,84 kB) | **455,58 kB** (gzip 132,03 kB) | **456,92 kB** (gzip 132,43 kB) | **457,00 kB** (gzip 132,51 kB) | **457,16 kB** (gzip 132,93 kB) | **457,75 kB** (gzip 133,12 kB) | **218,97 kB** (gzip 70,15 kB) | **219,12 kB** (gzip 70,19 kB) | **219,11 kB** (gzip 70,19 kB) | **219,52 kB** (gzip 70,35 kB) | 219,52 kB (gzip 70,35 kB) |
| Bundle: chunk do three.js, sob demanda | 618,87 kB (gzip 156,56 kB) | 618,87 kB (gzip 156,56 kB) | **618,91 kB** (gzip 156,58 kB) | **619,40 kB** (gzip 156,71 kB) | 619,40 kB (gzip 156,71 kB) | **619,48 kB** (gzip 156,73 kB) | **619,51 kB** (gzip 156,76 kB) | 619,51 kB (gzip 156,75 kB) | 619,51 kB (gzip **156,76 kB**) | **619,63 kB** (gzip **156,84 kB**) | 619,63 kB (gzip 156,84 kB) |
| Bundle: CSS | 20,76 kB (gzip 3,90 kB) | **21,75 kB** (gzip 4,04 kB) | **22,26 kB** (gzip 4,12 kB) | **22,69 kB** (gzip 4,21 kB) | **23,30 kB** (gzip 4,29 kB) | **23,81 kB** (gzip 4,37 kB) | **24,67 kB** (gzip 4,50 kB) | **24,77 kB** (gzip 4,50 kB) | **25,14 kB** (gzip **4,55 kB**) | 25,14 kB (gzip 4,55 kB) | 25,14 kB (gzip 4,55 kB) |
| Arquivos `.ts`/`.tsx` em `src` + `api` + `supabase` | 169 | 170 | 174 | **182** | **191** | **197** | **203** | **214** | **221** | **225** | **226** |
| Linhas de TypeScript nesses arquivos | 25.598 | 26.218 | 26.844 | **27.889** | **28.867** | **30.518** | **31.427** | **32.665** | **33.306** | **33.580** | **33.686** |
| `any`, `@ts-ignore`, `catch` vazio em `src/` e `api/` | zero de cada | zero de cada | zero de cada | zero de cada | zero de cada | zero de cada | zero de cada | zero de cada | zero de cada | zero de cada | zero de cada |
| `console.log` fora de teste | 1, proposital | 1, proposital | 1, proposital | 1, proposital | 1, proposital | 1, proposital | 1, proposital | 1, proposital | 1, proposital | 1, proposital | 1, proposital |
| Linter configurado | nenhum | nenhum (A13, no backlog) | nenhum (A13, no backlog) | nenhum (A13, no backlog) | nenhum (A13, no backlog) | nenhum (A13, no backlog) | nenhum (A13, no backlog) | nenhum (A13, no backlog) | nenhum (A13, no backlog) | nenhum (A13, no backlog) | nenhum (A13, no backlog) |
| CI | nenhum | nenhum (A14, no backlog) | nenhum (A14, no backlog) | nenhum (A14, no backlog) | nenhum (A14, no backlog) | nenhum (A14, no backlog) | nenhum (A14, no backlog) | nenhum (A14, no backlog) | nenhum (A14, no backlog) | nenhum (A14, no backlog) | nenhum (A14, no backlog) |
...
E os da rodada 9: chunk principal **+0,41 kB** (gzip +0,16 kB), que é o esboço, que mora no chunk
principal, passando a importar a frase do hex (A70); chunk do three.js **+0,12 kB**, que é o módulo
da medida do parâmetro (A68), dividido pelas duas telas tardias e posto pelo rolldown no chunk que
elas já dividem, ainda abaixo do limite de 640 kB do R8-A61. As duas telas tardias encolheram: a de
uma peça de 4,57 kB para 4,51 kB, a da composição de 28,45 kB para 28,12 kB. CSS igual. Os 14 testes
novos (1329 para 1343) saem de dois arquivos criados e quatro que ganharam casos; três testes de
frase mudaram de arquivo sem mudar de número. Nenhum item foi revertido.

E os da rodada 10: chunk principal, chunk do three.js e CSS iguais em kB; o chunk da tela da
composição foi de 28,12 kB para 28,10 kB. Os 3 testes novos (1343 para 1346) saem de um arquivo
criado e um que ganhou caso. O tempo do build variou entre 388 ms e 566 ms nas medições desta rodada
e da anterior, com a máquina fazendo outras coisas, e não é número de ganho nem de perda. Nenhum item
foi revertido.
