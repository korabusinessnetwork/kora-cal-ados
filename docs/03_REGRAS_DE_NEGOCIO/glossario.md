...
|---|---|---|---|
| **Zona** | `zone_key`, `product_zones` | Parte endereçável de um modelo de calçado (ex: sola, cabedal, cadarço, logo) que pode ter cor/material definidos independentemente. Em **produto trazido** ela nasce de alguém **marcar** clicando; em **produto gerado** ela nasce pronta, porque cada **peça** do acervo já é uma zona (ADR-008 D3). O conceito é o mesmo nos dois — o que muda é quem a produziu | frame, layer, área, region, parte (isolado), peça (peça é o ativo do acervo; zona é o papel dele num produto) |
| **Produto** / **Modelo** | `products` | Um modelo de calçado cadastrado no sistema, com um conjunto de zonas. Existe em dois modos que **convivem** (ADR-008 D5): **produto trazido**, que tem um asset-base (SVG ou glTF) que a marca subiu, e **produto gerado**, que tem uma **composição** sobre o acervo em vez de asset-base próprio | item, artigo, referência |
...
| **Marcar zona** | `marcarZona` | A operação de dizer quais elementos do asset-base canônico formam uma zona, gravando uma linha em `product_zones`. É o caminho do **produto trazido** e continua inteiro; em **produto gerado** ela simplesmente não acontece, porque a peça já chega nomeada e com material próprio (ADR-008 D3). Marcar zona é custo, não valor — a melhor marcação é a que não precisa existir | desenhar, selecionar (isolado), recortar |
...
| **Acervo** | a biblioteca de peças — ADR-008 D1/D6 | O conjunto de **peças** de calçado em glTF de onde toda **composição** é montada. Tem duas camadas: **acervo base**, modelado ou licenciado pela Kora e visível a todo tenant, e **acervo do tenant**, privado sob RLS. A segunda não é extra: sem ela, a marca que subisse sua sola proprietária a estaria entregando ao concorrente que usa o mesmo sistema | biblioteca (isolado), catálogo (catálogo é da marca, não nosso), assets (isolado), banco de modelos |
| **Peça** | um item do acervo, em glTF | Uma malha reusável do acervo: **uma** sola específica, **um** cabedal específico. Modelada uma vez, normalizada uma vez (nome próprio e material próprio, ADR-007 D4/D5) e reusada em qualquer número de produtos. **Peça não é sinônimo de zona**: peça é o ativo da biblioteca, zona é o papel que ele cumpre dentro de um produto — a ligação entre os dois é que, no produto gerado, cada peça é uma zona | parte (isolado), componente, módulo, zona (são coisas diferentes), asset (isolado) |
| **Categoria de peça** | o lugar que a peça ocupa na composição | O papel que uma peça cumpre no calçado: sola, entressola, cabedal, cadarço, língua, ilhós, contraforte, biqueira, logo. Uma composição tem **uma peça por categoria** — duas peças na mesma categoria é estado inválido, recusado na validação | tipo de peça, slot, encaixe, parte (isolado) |
| **Composição** | ADR-008 D2 | O que define um calçado **gerado**: quais peças, em que cor/material, com que parâmetros. É JSON de algumas linhas e **não é arquivo 3D** — o 3D é montado no navegador a partir dela. É o análogo exato de `zone_colors` sobre um canônico: o projeto guarda a receita, nunca o resultado. Por isso gerar é barato, reprodutível e diffável | modelo montado, cena, projeto, receita (isolado), build |
| **Forma** | o molde do pé; agrupa o acervo — ADR-008 D4 | O molde sobre o qual as peças são modeladas, e o que decide se duas peças **encaixam**: peça só combina com peça da mesma forma. O acervo não é um monte de solas — é um conjunto de formas, cada uma com suas peças, e cada forma nova multiplica o trabalho de modelagem. **Nunca use “forma” no sentido de formato ou aparência** — neste projeto a palavra é sempre o molde; para o outro sentido escreva “silhueta” | last, molde (isolado), formato, silhueta, template |
| **Produto trazido** × **produto gerado** | ADR-008 D5 | Os dois modos que convivem. **Trazido**: a marca sobe o asset-base dela, o time marca as zonas, a API escala — é o que existe e está testado. **Gerado**: o designer descreve, o sistema monta uma composição sobre o acervo, e as zonas já vêm prontas. Um produto é um **ou** outro, nunca os dois, e nenhum produto existente é migrado | modo antigo/novo (nenhum dos dois é antigo), manual × automático, legado |
| **Modelo 3D canônico** | saída de `normalizarModelo3d` | O glTF depois da normalização: toda malha endereçável com **nome único** e **material próprio**, sem URI externa. É **imutável**, como o asset-base canônico do SVG — o editor lê e nunca regrava (ADR-005). Vale tanto para produto 3D trazido quanto para cada peça do acervo | glTF limpo, modelo tratado, sanitizado (isolado) |
| **Variação paramétrica** | parâmetros de peça na composição — ADR-008 D7 | Os números contínuos que cada peça aceita (altura de entressola, espessura de sola, largura de biqueira), aplicados como **transformação** e nunca como malha nova. É o que impede um acervo de peças discretas de parecer um kit de montar: “sola mais robusta” pode ser a mesma peça com outro número, sem precisar de outra sola no acervo | ajuste, customização (isolado), morph, escala (isolado) |
| **Prompt** | o texto que o designer escreve na tela da composição, ADR-008 D1 | A descrição em linguagem natural do calçado que se quer ("cano alto vermelho, sem cadarço"). É **entrada de usuário**, e a resposta a ele vira escolha de arquivo: por isso nunca chega ao palco sem passar por `validarComposicao`. Prompt não é composição: o prompt pede, a composição é o que foi escolhido | pedido (isolado), comando, instrução, descrição (isolado) |
| **Modelo de linguagem** | a interface `ModeloDeLinguagem` (`src/lib/composicao/`) | O componente que lê o prompt e o **catálogo para o modelo** e responde uma composição em JSON. É o único componente do sistema em que não se pode confiar por construção (ADR-008). Hoje o único que existe é o **gerador de prova**; um fornecedor de verdade é pago e depende de decisão do dono | IA (isolado, no código), LLM, GPT, chatbot, assistente |
| **Catálogo para o modelo** | saída de `montarCatalogoParaModelo` | O texto que o modelo de linguagem lê para saber quais peças existem: só as peças **da forma** em uso, com id, categoria, rótulo e faixa de cada parâmetro. É recortado do acervo visível ao tenant, então a peça privada de uma marca nunca entra no catálogo que vai para o modelo de outra (ADR-008 D6). **Não confundir com "catálogo" da marca**, que é o produto dela à venda | contexto, prompt de sistema, lista de peças (isolado) |
| **Gerador de prova** | `modeloDeLinguagemDeProva` | Um modelo de linguagem **falso**, sem IA, sem rede e sem custo, que entende palavras-chave do prompt e escolhe peças do catálogo que recebeu. Existe para a esteira prompt → composição funcionar enquanto nenhum fornecedor foi escolhido (D12). A tela diz que ele **não é IA**: afirmar o contrário violaria a regra de transparência | mock (isolado), IA de mentira, simulador, fake (no texto da tela) |
...
- **2026-09-09** (segunda entrada do dia) — entram os termos do **ADR-008**: **acervo**, **peça**, **categoria de peça**, **composição**, **forma**, **produto trazido × produto gerado**, **modelo 3D canônico** e **variação paramétrica**. Duas armadilhas de nomenclatura justificam metade das linhas. A primeira é **peça × zona**: no produto gerado cada peça É uma zona, e a tentação de tratar as duas palavras como sinônimo é enorme — mas peça é o ativo reusável do acervo e zona é o papel dele dentro de **um** produto; a mesma peça é zona em mil produtos ao mesmo tempo, e quem colapsar os dois nomes não consegue mais escrever essa frase. A segunda é **forma**, que em português é palavra comum para “formato” e aqui é o termo técnico do setor calçadista para o molde do pé — o termo que decide se uma sola encaixa num cabedal. Deixar as duas acepções soltas no mesmo projeto produziria “a forma da sola” significando duas coisas na mesma frase, então o sentido comum foi proibido e mandado para “silhueta”. No mesmo passe, três linhas existentes deixaram de estar certas e foram corrigidas: **zona** (que agora nasce de dois jeitos), **produto** (que tinha “um asset-base (SVG)” como se fosse o único caso) e **marcar zona** (que deixa de ser obrigatória no modo gerado)
